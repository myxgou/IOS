...
- (void)postPictureURLTweetWithFormat:(NSString *)format 
    ...
    if (clentip != nil) {
        ...
        [dic setObject:[self deviceIPAdress] forKey:@"clientip"];
    }
    if (picurl != nil) {
...
//        [self deleteAuthorizeDataInKeychain];
        if ([temp_delegate respondsToSelector:@selector(onAccessTokenExpired)]) {
            [temp_delegate performSelector:@selector(onAccessTokenExpired) withObject:nil];
        }
        return YES;
    }
    return NO;
...
- (BOOL)isLoggin
{
    return ((![self isAuthorizeExpired]) && [self isLoggedIn]);
}
...
- (void)logInWithDelegate:(id) requestDelegate  
    ...
    TCWBAuthorizeView *authorizeView = [[TCWBAuthorizeView alloc] initWithUrl:urlString WithDelegate:self];
    authorizeView.redirectURI = self.redirectURI;
    [authorizeView show];
...
- (void)logOutWithDelegate:(id) requestDelegate
{
    temp_delegate = requestDelegate;
    [self logOut];
    
    if ([temp_delegate respondsToSelector:@selector(onLoginOut)]) {
        [temp_delegate performSelector:@selector(onLoginOut) withObject:nil];
    }
}
